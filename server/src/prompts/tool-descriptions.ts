...
首次起草、重构主线或标题层级时，用 replace_document（默认）提交一篇完整 Markdown。只重写已有的一节时，用 replace_section：sectionPath 给从 H1 到目标标题的完整路径，sectionMarkdown 只给该标题下的新正文，不重复标题；其中只能有更低级子标题。局部重写依赖同一会话里已经生成的完整初稿和来源表；不确定当前初稿结构时不要猜，改用整篇重写。局部模式仍须传整篇完整 sources（无来源传 []），新正文含 CIT 时才提交该小节的 citationAudit。
