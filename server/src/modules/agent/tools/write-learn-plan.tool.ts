...
export function createWriteLearnPlanTool(
  ...
  return tool({
    description:
      '把学习规划的「目标 + 理解 + 篇目脉络」写入当前主题的 AI 草稿区，供所有者在左栏只读查看。调用前：目标明确、理解段已成文（立锚 + 因果拓扑），脉络提案已有序排好。工具只落库，不建任何节点——建篇是所有者的事。调用时**必须同时给出 changeSummary**：一句话说明本次规划相比现有改了什么（用户审批要看），缺它会被退回。',
