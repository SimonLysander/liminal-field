...
beforeAll(async () => {
  ...
  repo = new AgentMemoryRepository(memoryModel, {
    next: jest.fn().mockImplementation(() => Promise.resolve(++fenceSequence)),
  } as never);
}, 120_000);
...
describe('setTasks / getTasks', () => {
  ...
  it('setTasksFenced 只允许排序键更高的审批覆盖', async () => {
    const firstFence = '00000000000000000001:0000000001';
    const newerFence = '00000000000000000002:0000000001';

    await expect(
      repo.setTasksFenced('draft_fenced', [{ title: '第一版' }], firstFence),
    ).resolves.toBe(true);
    await expect(
      repo.setTasksFenced('draft_fenced', [{ title: '较新版' }], newerFence),
    ).resolves.toBe(true);
    await expect(
      repo.setTasksFenced('draft_fenced', [{ title: '过期重放' }], firstFence),
    ).resolves.toBe(false);

    await expect(repo.getTasks('draft_fenced')).resolves.toEqual([
      expect.objectContaining({ title: '较新版' }),
    ]);
  });

  it('setTasksFenced 不把其他唯一索引冲突伪装成过期提交', async () => {
    await repo.upsert({
      type: 'user',
      title: 'session:draft_title_conflict',
      content: '用户记忆',
    });

    await expect(
      repo.setTasksFenced(
        'draft_title_conflict',
        [{ title: '任务' }],
        '00000000000000000001:0000000001',
      ),
...
describe('session 首次创建的并发恢复', () => {
  const duplicateKey = () =>
    Object.assign(new Error('duplicate key'), { code: 11000 });

  it('upsertSession 与任务插入竞争时按 agentKey 重试内容更新', async () => {
    const memoryModel = {
      updateOne: jest
        .fn()
    ...
    await expect(
      repository.upsertSession('draft_race', '会话摘要'),
    ).resolves.toBeUndefined();
    expect(memoryModel.updateOne).toHaveBeenCalledTimes(2);
    expect(memoryModel.updateOne.mock.calls[1][0]).toEqual({
      type: 'session',
      agentKey: 'draft_race',
    });
  });

  it('setTasksFenced 与会话插入竞争时重试条件更新，不误判 superseded', async () => {
    const memoryModel = {
      updateOne: jest
        .fn()
        .mockRejectedValueOnce(duplicateKey())
