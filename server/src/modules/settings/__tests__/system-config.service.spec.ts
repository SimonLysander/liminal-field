...
describe('SystemConfigService — 内置 agent 合成解析', () => {
  it('getAgentConfig 内置 key + 空 Mongo → 文件为准、provider 空、builtin=true', async () => {
    ...
    const gallery = await service.getAgentConfig('gallery-caption-writer');
    expect(gallery?.systemPrompt).toBe(
      'prompt:agents/gallery-caption-writer.md',
    );
    expect(gallery?.systemPrompt).not.toContain(
      'user-facing-prose-contract.md',
    );
  });
