  ...
  private async seedLearningSkills(): Promise<void> {
    ...
    if (!notePlan) {
      notePlan = await this.skillService.create({
        ...
        // 正文单一真源在 prompts/skills/note-plan.md（长文走 md，PromptManager 渲染）
        body: this.promptManager.render('skills/note-plan.md'),
        requiredTools: [],
      });
    ...
    if (!noteWriting) {
      noteWriting = await this.skillService.create({
        ...
        // 正文单一真源在 prompts/skills/note-writing.md（含引用规则；长文走 md，PromptManager 渲染）
        body: this.promptManager.render('skills/note-writing.md'),
    ...
    // 引用规则已并入 prompts/skills/note-writing.md;load_skill 经 SkillService 文件优先解析,
    // 内置 note-writing body 实时取自文件,无需再对 Mongo 存量做 body 迁移。
