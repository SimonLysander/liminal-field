...
export class SystemConfigService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    // 内置 agent / skill 不再 seed 进 Mongo:它们「是什么」定义在 prompts/builtin-agents.ts
    // + builtin-skills.ts + agents|skills/*.md,由 getAgentConfig / SkillService 文件优先
    // 合成解析(内置优先、线上不可改)。Mongo 只存用户新建的 agent/skill,以及内置 agent 的
    // provider 绑定与 enabled 开关(管理员 UI 运行时可调)。故启动不再做任何 seed / 工具补齐迁移。
  }
