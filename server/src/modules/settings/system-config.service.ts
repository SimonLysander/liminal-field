...
- 正文标题只用 H1–H3(篇名不在正文里,不必为它留级):主脉络分段用 H1,节内细分用 H2、再细用 H3,最多三级。
- 分节是给读者一副可扫的骨架,不是把文章剁碎:按思想的大转折分,通常一篇 3–6 节;一节里的多个小主题靠行文承接,别每冒出一个名词就起一个标题。标题标这节讲什么(如「光圈:进光的守门人」),不用「立锚/建模/兑现」这类工序名。
- 自然分段:一段一个意思,推进一步即另起一段;不要几百字壅成一堵墙,段间留空行。
- 并列项或多维对比才用列表、表格,由逻辑决定形式,不为排版而排版;关键术语首次出现可轻度强调,不在句中频繁加粗。
