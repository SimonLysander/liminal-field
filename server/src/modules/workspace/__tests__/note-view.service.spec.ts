...
describe('NoteViewService', () => {
  ...
  beforeEach(() => {
    ...
    minioService = {
      ...
      promoteDraftAssets: jest.fn().mockResolvedValue(undefined),
  ...
  describe('getHistory()', () => {
    it('从 ContentSnapshot 版本模型读取历史', async () => {
      const history = [{ commitHash: 'abc', message: 'init' }];
      contentService.getContentHistory.mockResolvedValue(history as any);

      const result = await service.getHistory('ci_1');

      expect(contentService.getContentHistory).toHaveBeenCalledWith('ci_1');
  ...
  describe('getByVersion()', () => {
    it('委托 contentService.getContentByVersion', async () => {
      const detail = { id: 'ci_1', title: 'v1', bodyMarkdown: '# 正文' };
  ...
  describe('uploadAsset()', () => {
    it('确认可编辑 + 切换工作分支后存储附件', async () => {
      ...
      expect(result).toEqual({
        url: 'content/ci_1/assets/img.png',
        fileName: 'img.png',
        contentType: 'image/png',
        size: 4,
      });
