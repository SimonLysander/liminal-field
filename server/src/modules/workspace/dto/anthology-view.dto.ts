 ...
 *
 * 设计:展示端只暴露已发布信息;条目列表 DTO 不含正文(减少流量),详情 DTO 才含正文。
 ...
 * - 条目级发布:子 ContentItem.publishedVersion(每个子节点各自发布)
 * - 展示端只能看到文集已发布 + 条目 publishedVersion 非 null 的子节点
 */
...
export class AnthologyPublicDetailDto {
  ...
  bodyMarkdown: string;
  /** 条目列表(不含正文),仅包含 publishedVersion 非 null 的已发布子节点。 */
...
export class AnthologyEntryDetailDto {
  ...
  updatedAt: string;
  bodyMarkdown: string;
  /** 上一篇(按索引顺序),null 表示已是第一篇。nodeId = 子 contentItemId。 */
  prev: { nodeId: string; title: string } | null;
  /** 下一篇(按索引顺序),null 表示已是最后一篇。nodeId = 子 contentItemId。 */
