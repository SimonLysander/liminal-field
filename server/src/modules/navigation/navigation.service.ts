...
export class NavigationNodeService {
  ...
  async getFolderOverview(folderId: string): Promise<FolderOverviewDto> {
    ...
    for (const child of children) {
      ...
      const childIsFolder = (childCounts[child._id.toString()] ?? 0) > 0;
      if (childIsFolder) {
        // 有子节点的子项当作"文件夹"：递归统计子树
        dto.type = 'FOLDER';
        directFolderCount++;
        const descendants = await this.navigationRepository.findAllDescendants(
          dto.id,
        );
        // 节点同质化:子树里的"文档"= 叶子(在该子树内不作为任何节点的父)
        const descParentIds = new Set(
          descendants
            .map((d) => d.parentId?.toString())
            .filter((p): p is string => !!p),
        );
        const docDescendants = descendants.filter(
          (d) => !descParentIds.has(d._id.toString()),
        );
