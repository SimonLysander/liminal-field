...
describe('全流程:建→发布→推远端→清→恢复→重发 (e2e, 本地 bare 仓当远端)', () => {
  ...
  it('内容穿越「发布→推→清→恢复→重发」全程,发布状态不进 Git、恢复后重置、草稿被清', async () => {
    ...
    // 子节点 ContentItem(childNodeId 即其 id)各自上线
    expect((await repo.findById(childNodeId))?.publishedVersion).toBeTruthy();
    ...
    // 子节点是独立子 ContentItem(childNodeId 即其 ci 目录);正文写在它自己的 main.md
    expect(idsInRemote).toContain(childNodeId);
    const entryMd = await readFile(
      join(cloneContent, childNodeId, 'main.md'),
      'utf8',
    );
    ...
    // ── 4. 播一条草稿(子节点 ContentItem 的普通草稿,fileName=null),再从远端恢复 ──
    await draftModel.create({
      _id: `draft:${childNodeId}`,
      contentItemId: childNodeId,
      bodyMarkdown: '草稿中…',
      title: '条目甲',
      changeNote: 'wip',
      savedAt: new Date(),
    ...
    expect(
      (await repo.findById(childNodeId))?.publishedVersion ?? null,
    ).toBeNull();
