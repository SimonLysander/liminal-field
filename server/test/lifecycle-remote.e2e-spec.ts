 ...
 * - 条目正文 entries/*.md 真的进 Git 并穿越远端往返(P2 + 远端腿)
 * - 发布状态不进 Git、恢复后重置为未发布(A 重构)
...
describe('全流程:建→发布→推远端→清→恢复→重发 (e2e, 本地 bare 仓当远端)', () => {
  ...
  it('内容穿越「发布→推→清→恢复→重发」全程,发布状态不进 Git、恢复后重置、草稿被清', async () => {
    ...
    await commitNoteContent(ctx.app, cookie, noteId, '# 笔记正文\n\n内容。', '生命周期笔记');
    const anthologyId = await createAnthologyItem(ctx.app, cookie, '生命周期文集');
    ...
    expect(anthBefore?.entryPublishStates?.length).toBe(1);
    ...
    const remoteBranches = execFileSync('git', ['-C', cloneDir, 'branch', '-r'], {
      encoding: 'utf8',
    });
    ...
    execFileSync('git', ['-C', cloneDir, 'checkout', wsRemote!.replace('origin/', '')]);
    ...
    // 条目正文 entries/*.md 进了 Git
    const entryMd = await readFile(
      join(cloneContent, anthologyId, 'entries', `${entryKey}.md`),
      'utf8',
    );
    expect(entryMd).toContain(ENTRY_BODY);
    // 索引 main.md 含条目结构、但【不含】发布状态
    const mainMd = await readFile(join(cloneContent, anthologyId, 'main.md'), 'utf8');
    expect(mainMd).toContain(entryKey);
    ...
    await draftModel.create({
      _id: `draft:${anthologyId}:entries/${entryKey}.md`,
      contentItemId: anthologyId,
      bodyMarkdown: '草稿中…',
      title: '条目甲',
      changeNote: 'wip',
      savedAt: new Date(),
    ...
    expect(anthAfter?.entryPublishStates ?? []).toHaveLength(0);
    // 条目正文恢复(管理端读取)
    ...
    expect((await repo.findById(anthologyId))?.entryPublishStates?.length).toBe(1);
  }, 180_000);
});
