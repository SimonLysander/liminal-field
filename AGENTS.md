...
## 生产机禁止构建镜像和全量扫描 Docker 存储

**背景：** 生产机只有 1.8 GiB 内存。曾在部署后执行 `docker system df`，Docker 客户端长期阻塞并使 `dockerd` 内存持续增长，最终触发全局 OOM，导致整套容器退出。

**规则：**
1. 前后端镜像必须在本地按生产架构构建并传到服务器；线上只允许加载镜像和定向重建容器，不运行 `docker compose build`。
2. 线上不得运行 `docker system df`、`docker system prune` 等全量扫描或清理命令。磁盘检查使用 `df` 和面向明确目录的只读检查；清理必须指定已确认的对象。
3. 部署后必须验证全部 Compose 服务状态、容器重启次数、公开站点和 API；不得只验证本次修改的单个服务。
4. 生产容器使用 `restart: always`，保证 Docker 守护进程重启后恢复服务。
